...
namespace eval ::Nanotube:: {
    ...
    variable ccbond 0.1418; # length of C-C bond in nanometers
...
proc ::Nanotube::nanotube_gui {} {
    ...
    incr row
    grid [label $w.cclabel -text "Length of C-C bond (nm): "] \
    ...
    grid [button $w.gotube -text "Generate Nanotube" \
              -command [namespace code \
                            { vmdcon -info \
                                  "calling nanotube_core -l $l -n $n -m $m -b $bonds -a $angles -d $dihedrals -i $impropers -cc $ccbond"
                              nanotube_core -l "$l" -n "$n" -m "$m" -b "$bonds" \
                                  -a "$angles" -d "$dihedrals" -i "$impropers" -cc "$ccbond"
                            } ]] -row $row -column 0 -columnspan 4 -sticky nsew
    incr row
    ...
    grid [button $w.gosheet -text "Generate Sheet(s)" \
              -command [namespace code \
                            { vmdcon -info \
                                  "calling graphene_core -lx $lx -ly $ly -type $type -nlayers $nlayers -b $bonds -a $angles -d $dihedrals -i $impropers -cc $ccbond"
                              graphene_core -lx "$lx" -ly "$ly" -type "$type" \
                                  -nlayers "$nlayers" -b "$bonds" -a "$angles" \
                                  -d "$dihedrals" -i "$impropers" -cc "$ccbond"
                            } ]] -row $row -column 0 -columnspan 4 -sticky nsew
}
