#
# $Id: fftk_GenZMatrix.tcl,v 1.11 2012/10/18 03:03:18 mayne Exp $
#
#======================================================
...
proc ::ForceFieldToolKit::GenZMatrix::genZmatrix {} {
    ...
    foreach donorAtom $donList {
		...
		# open output file
		set outname "${outFolderPath}/${basename}-DON-${donorName}.gau"
    ...
    foreach acceptorAtom $accList {
		...
		# open output file
		set outname "${outFolderPath}/${basename}-ACC-${acceptorName}.gau"
		set outfile [open $outname w]
...
proc ::ForceFieldToolKit::GenZMatrix::writeExceptionZMats { aName aInd gnames atom_info } {
	...
	if { $aElem eq "O" && ($bElem eq "C" || $bElem eq "P" || $bElem eq "S") && [llength $bondlistA] == 1 } {

		# write two slightly different files
		foreach altPos {"a" "b"} dihed {180 0} {

			# open output file
			set outname "${outFolderPath}/${basename}-ACC-${aName}-120${altPos}.gau"
			set outfile [open $outname w]
...
proc ::ForceFieldToolKit::GenZMatrix::writeSPfiles {} {
    ...
    # open output file
    set outfile [open ${outFolderPath}/${basename}-sp.gau w]

    # write the header
    puts $outfile "%chk=${basename}-sp.chk"
    puts $outfile "%nproc=$qmProc"
    puts $outfile "%mem=${qmMem}GB"
    puts $outfile "# RHF/6-31G* SCF=Tight"
    puts $outfile ""
    puts $outfile "<qmtool> simtype=\"Single point calculation\" </qmtool>"
    puts $outfile "${basename}-sp"
    puts $outfile ""
    puts $outfile "$qmCharge $qmMult"
    ...
    # write TIP3P sp GAU file
    set outfile [open ${outFolderPath}/wat-sp.gau w]
