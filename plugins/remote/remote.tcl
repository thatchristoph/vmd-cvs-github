...
#   puts "Sending the button configuration to $currentUserList"

   # msg type 2 is 'buttons'
   foreach client $currentUserList {
      ::mobile sendMsg [lindex $client 0] [lindex $client 1] \
                                          2 [string trim $strBuild]
   }
...
# by eventually requiring something to be in the request
    ...
    if { [info exists webserverHash($req)] && \
         [file exist $webserverHash($req)] } {
      set fp [open $webserverHash($req) rb]
      set data [read $fp]
      close $fp

      set mimeType [getMimetype $webserverHash($req)]
...
# -------------------------------------------------------------------------
proc ::remote::getWwwFileHash { path } {
   variable webserverHash

   set num [getSingleHashKey webserverHash]

   set webserverHash($num) $path
   return $num
}
...
proc ::remote::distributeImageToAll { fname title } {
  set currentUserList [::mobile get clientList]
  foreach client $currentUserList {
     distributeImage [lindex $client 0] [lindex $client 1] $fname $title
  }
}

# -------------------------------------------------------------------------
proc ::remote::distributeImage { clientNick clientIp fname title } {
   variable serverSocket
# set up webserver, if not already set up
   ...
   set msg [list $title [getWwwFileHash $fname] ]

   # notify client(s)
   # '1' means that the message is a file that 
   #  there is a file that needs to be picked up
...
# this numberToSend could be an index into a lookup table 
# so that we can eventually send all kinds of things
# (and not just images)
...
# set up webserver, if not already set up
   ...
   ::mobile sendMsg $clientNick $clientIp 0 [getWwwFileHash $fname]
}
