...
//    ToggleButton button0 = null;
    ...
    final int notInControlColor = 0xff000000 
              + 50 * 0x10000
              +  0 * 0x100
              +  0;
    final int inControlColor = 0xff000000 
              +  0 * 0x10000
              + 50 * 0x100
              +  0;
    int currentColor = notInControlColor;
...
//       logConsole("button" + id + ",val: " + value +"," + buttonState);
   ...
   // ----------------------------------------------------------------
   public void packetReceived(final byte [] pkt, final String kstrAddr)
   {
...
//         Log.d("UDPServer", "Nick length:" + nickLength);
         ...
         String str = new String(pkt, offset, nickLength);
         offset += nickLength;
...
//          numConnections + "," + strClients);
      ...
      switch (event) {
         case SERVER_MESSAGE:
            ...
            String str = new String(pkt, offset, msgLength);
            offset += msgLength;

            switch (msgType) {
               case 0:                                        // snapshot
                  // we need to get the snapshot at http://server:5141/str
                  addSnapshot( "http://" + mPrefs.getString("ipEntry", "") 
                               + ":5141/" +  str);
                  break;
...
//                  logConsole("str is '" + str + "'");
                  ...
                  addImageTab( "http://" + mPrefs.getString("ipEntry", "") 
                                                    + ":5141/" +  arr[1],
                               "snap" + imgTabNum++,
                               arr[0],
                               false);
...
//        Log.d(tag, "onResume start");
        m_udpServer  = new UDPServer(this, 
                     Integer.parseInt(mPrefs.getString("listenportEntry",
                       "" + DEFAULT_LISTEN_PORT)));
        m_udpServer.start();
   ...
   // ----------------------------------------------------------------
    ...
    private void initialPack(byte [] array)
    {
       ...
       if (m_iApiVersion >= 9)
       {
          ...
          packInt( Integer.parseInt(mPrefs.getString("listenportEntry",
                       "" + DEFAULT_LISTEN_PORT)), array, 24);
