 ...
 *      $Revision: 1.34 $       $Date: 2012/07/19 20:00:14 $
 *
 ***************************************************************************/
...
static int read_wave_coeffs(FILE *file, qm_wavefunction_t *wave_a, qm_wavefunction_t *wave_b);
 ...
 * In case there are both sections then the coordinates of
 * the first frame in the [GEOMETRIES] section are 
 * identical to the coordinates in the [ATOMS] section.
 * Each coordinate set in [GEOMETRIES] lists the # atoms
 * in its first line. For the [ATOMS] section we have
 * to count lines to get the # atoms.
 *
...
static int get_basis(qmdata_t *data) {
  ...
  for (i=0; i<data->num_shells; i++) {
    ...
    case SP_P_SHELL:
      ...
      j += 9;
      if (data->max_shell<P_SHELL) data->max_shell = P_SHELL;
      break;
    case D_SHELL:
      ...
      j += 18;
      if (data->max_shell<D_SHELL) data->max_shell = D_SHELL;
      break;
    case F_SHELL:
      ...
      j += 30;
      if (data->max_shell<F_SHELL) data->max_shell = F_SHELL;
      break;
    case G_SHELL:
      ...
      j += 45;
      if (data->max_shell<G_SHELL) data->max_shell = G_SHELL;
      break;
    }
  }
 ...
 ***********************************************************/
static int count_orbitals(qmdata_t *data) {
  int num_orbs_alpha=0, num_orbs_beta=0;
  ...
  /* Count alpha/beta orbitals */
  while (1) {
    int i;
    spin[0] = '\0';
    for (i=0; i<4; i++) {
      char buffer[1024], val[128];
      if (!fgets(buffer, 1024, data->file)) return FALSE;
      if (!sscanf(buffer," Sym= %s", val) &&
	  !sscanf(buffer," Ene= %f", &orbenergy) &&
	  !sscanf(buffer," Spin= %s", spin) &&
	  !sscanf(buffer," Occup= %f", &occu)) break;
    }

    if      (!strcasecmp(spin, "ALPHA")) num_orbs_alpha++;
    else if (!strcasecmp(spin, "BETA" )) num_orbs_beta++;
    else {
      printf("moldenplugin) Orbital count failed. Stumbled over 'Spin= %s.\n", spin);
      return FALSE;
    }

    if (!goto_keyline(data->file, "= ", NULL)) break;
  }
  ...
  if (num_orbs_alpha) {
    ...
    wave->exci = 0;
    wave->mult = 1;
    wave->num_coeffs = data->wavef_size;
    wave->num_orbitals = num_orbs_alpha;
  }
  ...
  if (num_orbs_beta) {
    ...
    wave->mult = 1;
    wave->num_coeffs = data->wavef_size;
    wave->num_orbitals = num_orbs_beta;
  }
...
static int read_molecular_orbitals(qmdata_t *data) {
  moldendata_t *moldendata = (moldendata_t *)data->format_specific_data;
  qm_wavefunction_t *wave_a = NULL;
  qm_wavefunction_t *wave_b = NULL;

  if (!data->qm_timestep) return FALSE;
  if (!data->qm_timestep->numwave) return FALSE;

  /* Allocate memory for spin ALPHA wavefunction */
  if (data->qm_timestep->wave[0].num_orbitals) {
    wave_a = &data->qm_timestep->wave[0];
    ALLOCATE(wave_a->wave_coeffs, float, wave_a->num_coeffs*wave_a->num_orbitals);
    printf("Spin alpha:\n");
    printf("num_coeffs   = %d\n", wave_a->num_coeffs);
  ...
  /* Allocate memory for spin BETA wavefunction */
  if (data->qm_timestep->numwave>1 &&
      data->qm_timestep->wave[1].num_orbitals) {
    wave_b = &data->qm_timestep->wave[1];
    ALLOCATE(wave_b->wave_coeffs, float, wave_b->num_coeffs*wave_b->num_orbitals);   
    printf("Spin beta:\n");
    printf("num_coeffs   = %d\n", wave_b->num_coeffs);
    printf("num_orbitals = %d\n", wave_b->num_orbitals);
    printf("num_wave     = %d\n", data->qm_timestep->numwave);
  ...
  printf(" Read wavefunction coefficients\n");

  /* Place file pointer on line after the [MO] keyword. */
  fseek(data->file, moldendata->filepos_mo, SEEK_SET);

  /* Read wavefunction coefficients */
  if (!read_wave_coeffs(data->file, wave_a, wave_b)) return FALSE;
...
static int read_wave_coeffs(FILE *file, qm_wavefunction_t *wave_a, qm_wavefunction_t *wave_b) {
  int i, j, nr;
  int num_coeffs, num_orb=0, iorb=0;
  int iorb_a = 0;
  int iorb_b = 0;
  char buffer[1024];
  float *wave_coeffs;
  ...
  if (wave_a) {
    printf("# alpha orb = %d\n", wave_a->num_orbitals);
    num_orb = wave_a->num_orbitals;
  }
  if (wave_b) {
    printf("# beta  orb = %d\n", wave_b->num_orbitals);
    num_orb += wave_b->num_orbitals;
  }
  ...
  for (i=0; i<num_orb; i++) {
    ...
    /* Find first line of coefficients */
    for (nr=0; nr<4; nr++) {
      char buffer[1024], sym[128];
      float val;
      if (!fgets(buffer, 1024, file)) return FALSE;
      if (!sscanf(buffer," Sym= %s", sym) &&
	  !sscanf(buffer," Ene= %f", &val) &&
	  !sscanf(buffer," Spin= %s", spin) &&
	  !sscanf(buffer," Occup= %f", &val)) break;
    }

    if (!strcasecmp(spin, "ALPHA")) {
      wave_coeffs = wave_a->wave_coeffs;
      num_coeffs  = wave_a->num_coeffs; 
      iorb = iorb_a++;
    }
    else if (!strcasecmp(spin, "BETA")) {
      wave_coeffs = wave_b->wave_coeffs;
      num_coeffs  = wave_b->num_coeffs;
      iorb = iorb_b++;
     } 
    else {
      printf("moldenplugin) Found orbital with bad spin value. 'Spin= %s.\n", spin);
      return FALSE;
    }
    ...
    for (j=0; j<num_coeffs; j++) {
      ...
      float coeff;
      if (!fgets(buffer,1024,file)) return FALSE;
      nr = sscanf(buffer,"%d %f", &icoeff, &coeff);

      if (nr<2) break;

      wave_coeffs[iorb*num_coeffs+icoeff-1] = coeff;
    }
  }
