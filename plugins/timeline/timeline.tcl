...
#
#
# $Id: timeline.tcl,v 1.84 2012/08/10 15:18:42 barryi Exp $
...
proc ::timeline::setParamsInterSelContacts {args} {
  ...
  label $d.head -justify center -relief raised -text {Inter-selection contact parameters:}
    ...
    label $d.lb  -justify left -text {Selection list:}
    set i 1
    grid columnconfigure $d.top 0 -weight 2
    foreach l "$d.la $d.lb" {
        pack $l -in $d.top -side left -expand 1 -padx 3m -pady 3m
    ...
    entry $d.eb  -justify left -textvariable ::timeline::interSelList
    set i 1
    grid columnconfigure $d.top 1 -weight 2
    foreach l "$d.ea $d.eb" {
        pack $l -in $d.top -side left -expand 1 -padx 3m -pady 3m
    ...
    # buttons
    set com "::timeline::calcInterSelContacts ${args}; set ::timeline::clicked 1"
    button $d.ok -text {OK} -command $com

    grid $d.ok -in $d.bot -column 0 -row 0 -sticky ew -padx 10 -pady 4
    grid columnconfigure $d.bot 0
    button $d.cancel -text {Cancel} -command {set ::timeline::interDist $::timeline::oldInterDist; set ::timeline::interSelList $::timeline::oldInterSelList; set ::timeline::clicked 1}
    grid $d.cancel -in $d.bot -column 1 -row 0 -sticky ew -padx 10 -pady 4
    grid columnconfigure $d.bot 1
...
proc ::timeline::setParamsGlobalCcss {args} {
 ...
 # save old values 
  ...
  toplevel $d -class Dialog
  wm title $d {Set Global cross corr. sec. struct  parameters for Timeline}
  wm protocol $d WM_DELETE_WINDOW {set ::timeline::clicked -1}
  wm minsize  $d 320 120  
  ...
  label $d.head -justify center -relief raised -text {Global cross corr. sec. struct parameters:}
    ...
    label $d.lf  -justify left -text {Map threshold:}
    set i 1
    grid columnconfigure $d.top 0 -weight 2
    foreach l "$d.lc $d.ld $d.le $d.lf" {
        pack $l -in $d.top -side left -expand 1 -padx 3m -pady 3m
        grid $l -in $d.top -column 0 -row $i -sticky w 
        incr i
    }
    ...
    entry $d.ef  -justify left -textvariable ::timeline::ccssThreshold
    set i 1
    grid columnconfigure $d.top 1 -weight 2
       # set ip file name buttons
    ...
    # buttons
        set com "::timeline::calcGlobalCcss ${args}; set ::timeline::clicked 1"
    button $d.ok -text {OK} -command $com

    grid $d.ok -in $d.bot -column 0 -row 0 -sticky ew -padx 10 -pady 4
    grid columnconfigure $d.bot 0
    button $d.cancel -text {Cancel} -command {set ::timeline::ccssMapFile $::timeline::oldCcssMapFile;  set ::timeline::ccssSpacing $::timeline::oldCcssSpacing; set ::timeline::ccssThreshold $::timeline::oldCcssThreshold; set ::timeline::clicked 1}
    grid $d.cancel -in $d.bot -column 1 -row 0 -sticky ew -padx 10 -pady 4
    grid columnconfigure $d.bot 1
...
proc ::timeline::draw_interface {} {
 ...
 tlPutsDebug ": about to register calc menus"
  ...
  $w.menubar.calculate.menu add command -label "Calc. cross-corr. (sec. struct)..." -command  [namespace code {setParamsGlobalCcss 22; postCalc;}] 
...
proc ::timeline::calcNative {} {
  clearData; calcNativeProperty 19
}
...
proc ::timeline::calcInterSelContacts {lastCalcVal} {
  ...
  #validate selList to validSelList
  tlPutsDebug "length of selList is [llength $selList]" 
  set validSelList "" 
  foreach selString $selList {
    ...
    if {[catch {set sel [atomselect $currentMol "$selString"]; set selNum [$sel num] } ] } {
      puts "VMD Timeline Warning: Can't make atom selection for: $selString"
    } else {
      if {$selNum > 0} {
        ...
        puts "VMD Timeline Warning: Can't find atoms for selection: $selString"
      }
    }
  } 
...
# of secondary structural segments
  ...
  set calcCountTotal [expr  ($lastAnalysisFrame - $firstAnalysisFrame +1)]
  set calcCount 0
  tlPutsDebug "starting globalCC, first line"
  ...
  set dataName(vals) "cross-corr. ss"
  ...
  set resNum [llength [lsort -unique [$atomSel get resid]]]
  tlPutsDebug "residue number = $resNum"
    ...
    foreach seg $segList {
      ...
      for {set id_i $id_begin} {$id_i <= $id_end} {incr id_i} {
        ...
        if {$ss_0 != $ss_1} {
          ...
          tlPutsDebug "current CCSS = $CCSS"
          tlPutsDebug check4.1
          # PDB beta stores raw CC
          #$ssSel set beta $CCSS
          # dirty hack, write y
          #$ssSel set y $CCSS
          ...
          set scaling [expr {double($id_i - $id_0 + 1) / double($resNum)}]
          tlPutsDebug "scaling=[expr {double($id_i - $id_0 + 1) / double($resNum)}]"

          #Now get scaled quantity (this is the one we want)
          set CCwSS [expr $CCSS * $scaling]
           

          # PDB occupancy stores weighted CC
          #$ssSel set occupancy $CCwSS
          # dirty hack, wirte z
          # CONVERT: $ssSel set z $CCwSS
          ...
          foreach elem $setList {
            foreach {aName aResid aChain aSegname} $elem {
              ...
              if {$aName=="CA"} {
                ...
                set ccList($aResid,$aChain,$spaceToCodeSeg) $CCwSS
 ...
 # Now loop through atoms in the selection as for a normal function lookup and look up  in the compiled ccList
    ...
    set percent [expr {1 - double($nanRegister) / double($resNum)}]
