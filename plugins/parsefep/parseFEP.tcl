...
#                    - inaccuracy_estimation. the overlap between the forward and backword simulations will be estimated.
...
#######################################################################
# Chris Chipot, Liu Peng, 2007-2012, [ CV: 2012.08.27 ]
#######################################################################
...
variable   max_order  ""
variable   gaussian_guess 0  ;# 0 false; 1 true
...
#Input for temperature
	...
	grid [entry $w.temp2 -width 5 \
		...
		-row $row -column 1 -sticky w
	incr row
	grid [checkbutton $w.dispindex -text      "disp (this option is restricted to Unix-like systems)   " -variable [namespace current]::dispindex] -row $row -column 0 -sticky w
	grid [checkbutton $w.entropy -text   "entropy" -variable [namespace current]::entropyindex ] -row $row -column 1 -sticky w
	grid [checkbutton $w.gg -text   "Gaussian approximation" -variable [namespace current]::gaussian_guess ] -row $row -column 2 -sticky w
...
#####################
	...
	if { $fororback == "backward"  && $window == 1 && $::ParseFEP::entropyindex == 1 } {
		set tempn [open entropy.log "a+"]
                puts $tempn [format "backward: entropy         %8.4f      %8.4f      %8.4f      %8.4f      %8.4f"   1.0  0.0     $::ParseFEP::fep_delta_u_backward   0.0  $::ParseFEP::fep_delta_s_backward ]
		close $tempn
	}
...
##################################################################################
	...
	if { $::ParseFEP::entropyindex == 1 }	{
		...
		set i           [lindex [lindex $file_lambda [expr  $window  -1 ] ] 7 ]
		if { $fororback == "forward" } {
	...
	puts [format "$fororback:                     ∆σ²                                  = %7.4f"   $instant_variance ]
	puts [format "$fororback:                     σ²                                   = %7.4f"    $::ParseFEP::variance ]
	puts [format "$fororback:                     ∆δε                                  = %7.4f"   $instant_error ]
...
#  GAUSSIAN CASE
	if { $::ParseFEP::gaussian_guess } {
		...
		set ::ParseFEP::square_error_gauss 	 [expr $::ParseFEP::square_error_gauss + $instant_error_gauss ]
	        ...
	        if { $instant_error_gauss > 0 } {
			set instant_error_gauss [expr sqrt($instant_error_gauss)]
	        } else {
			set instant_error_gauss 0.
		}	

	        if { $::ParseFEP::square_error_gauss > 0 } {
	        	set ::ParseFEP::error_gauss  [expr sqrt($::ParseFEP::square_error_gauss) ]
        	} else {
	                set ::ParseFEP::error_gauss 0. 
        	}

		puts [format  "$fororback:                     ∆σ²(2nd-order)                       = %7.4f"  $instant_variance_gauss ]
		puts [format  "$fororback:                     σ²(2nd-order)                        = %7.4f"  $::ParseFEP::variance_gauss  ]
		puts [format  "$fororback:                     ∆δε(2nd-order)                       = %7.4f" $instant_error_gauss  ]
...
proc ::ParseFEP::inaccuracy_estimation {} {
		...
		puts "======================================================================================================"                     
		puts  "                   i          ideltai         P0_overlap_P1    forward_relative inaccuracy    backward_relative inaccuracy "            
		puts "======================================================================================================"   
...
###############################################################
				...
				puts  [format "inaccuracy:  %7.4f        %7.4f         %7.2f         %7.2f         %7.2f"  $i   $ideltai    $overlap   $f_rela_accu   $g_rela_accu  ]
		}
		puts "======================================================================================================"   
}
...
#######################################################################
	...
	foreach line $data_file {
		...
		if { [ regexp  "backward:" $line ] && [ regexp "entropy"  $line ]  } { 
			foreach { , , elem3 elem4 elem5 elem6 elem7 }  $line { break } 
