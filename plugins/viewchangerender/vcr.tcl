  ...
  if {$::MovieMaker::userframe < $::MovieMaker::numframes && $::MovieMaker::userframe > $::VCR::CurMovieMakerFrame} {
      if {$::VCR::tracking == 0} {
            if { $::VCR::movieListPos == 0 } { 
                ...
                puts "\ttotal time=[expr $::VCR::movieTime*$::VCR::movietimescale]"
...
proc ::VCR::init_parallelmovie { } {
        ...
        puts "Total Frames = $frames   Total Time = $totaltime"
        for { set i 0 } { $i < $::MovieMaker::numframes } { incr i } {
            ...
            puts "\[$fno\] startvp=$svp endvp=$evp step=$vpstep"
        }
        set ::VCR::totFrames $totFrames
        set ::VCR::framestep $::MovieMaker::trjstep
        puts "Generating frames with View-Change-Render in parallel."
        puts "Movie Maker settings:"
        puts "\tframerate=$::MovieMaker::framerate fps"
        puts "\tduration=$::MovieMaker::movieduration sec" 
        puts "\tframeskip=$::MovieMaker::trjstep"
        ...
        puts "\ttotal trajectory frames=$::VCR::totFrames"
        puts "\ttotal time=[expr $::VCR::movieTime*$::VCR::movietimescale]"
        puts "\tllength of start_vps = [llength $::VCR::movieStart_vp]"
        puts "\tRetrieving first viewpoint"
        trace add variable ::MovieMaker::abort write ::VCR::movieabortcallback
        trace add variable ::MovieMaker::statusstep write ::VCR::moviemakerDone_callback
        #::VCR::save_vp "movieinit"
...
proc ::VCR::parallelmoviecallback { args } {
  ...
  if {$::MovieMaker::userframe < [llength $::VCR::movieStart_vp] } {
      ...
      if {$::MovieMaker::userframe > 0 } {
        ...
        puts "\[$::MovieMaker::userframe\] In transition $startvp->$finalvp $tracking"
