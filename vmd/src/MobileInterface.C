 ...
 *	$Revision: 1.36 $	$Date: 2012/03/20 15:44:37 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
void Mobile::prepareSendBuffer(const int eventType)
{
  ...
  int caretLoc = 0;
  *(statusSendBuffer + caretLoc) = 1;    // endianism 
  caretLoc += sizeof(int);
  *(statusSendBuffer + caretLoc) = CURRENTAPIVERSION;    // version
  caretLoc += sizeof(int);

  // current mode in VMD (off, move, animate, tracker, etc)
  *(statusSendBuffer + caretLoc) = moveMode;    // off/move/animate/etc
  caretLoc += sizeof(int);

  // event code for what we are sending
  *(statusSendBuffer + caretLoc) = eventType;    
  caretLoc += sizeof(int);
  ...
  *(statusSendBuffer + caretLoc) = clientNick.num();    // how many connections?
  caretLoc += sizeof(int);
//  fprintf(stderr, "num connections: %d\n", clientNick.num());
  ...
  for (int i=0; i<clientNick.num();i++)
  {
     ...
     *(statusSendBuffer + caretLoc) = strLength;
     caretLoc += sizeof(int);
...
//     fprintf(stderr, "clientactive is '%d'\n", (clientActive[i] ? 1 : 0));
     *(statusSendBuffer + caretLoc) = (clientActive[i] ? 1 : 0);    // Is this nick active?
     caretLoc += sizeof(int);
  }
...
//    int port = 4141;
    ...
    for (int i=0; i< clientIP.num(); i++)
    {
       ...
       *(statusSendBuffer + 16) = (clientActive[i] ? 1 : 0);    // Is this nick active?
      ...
      if (touchinprogress && padaction == EVENT_TOUCH_MOVE) {
        if (touchmode == ROTATE) {
          ...
          if (moveMode == MOVE) {
            ...
            float rotby = (newrotangle-touchrotstartangle)*180.0f/VMD_PI;
...
//   fprintf(stderr, "Sending %s (%s) msgtype %s, msg '%s'\n",nick,ip,msgType,msg);
  ...
  if (found) {
    ...
    *(statusSendBuffer + 16) = (clientActive[i] ? 1 : 0);    // Is this nick active?
    ...
    // msgType must be an integer.  So, let's get the int.
    int tmpInt;
    if (EOF == sscanf(msgType, "%d", &tmpInt))
    {
       return false;
    }
    *(statusSendBuffer + length) = tmpInt;
    length += sizeof(int);

    int msgLength = strlen(msg);
    *(statusSendBuffer + length) = msgLength;
    length += sizeof(int);
