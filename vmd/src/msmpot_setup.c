 ...
 *      $Revision: 1.4 $      $Date: 2010/06/08 15:56:05 $
 *
 ***************************************************************************/
...
int Msmpot_configure(Msmpot *msm,
  ...
  else msm->errtol = DEFAULT_ERRTOL;
...
int setup_bins(Msmpot *msm) {
  ...
  int maxbin;
  int nbx = ceilf(msm->px / blen);  /* using ceilf to count bins */
  int nby = ceilf(msm->py / blen);  /* makes bin vol <= desired vol */
  int nbz = ceilf(msm->pz / blen);
