 ...
 *	$Revision: 1.64 $	$Date: 2011/02/01 16:46:35 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
void MoleculeGraphics::create_cmdlist(void) {
  ...
  for (int i=0; i<num; i++) {
    ...
    case MATERIAL: {
      ...
      cmdList->transmode = data[7];
      cmdList->materialtag = (int)data[8];
      break;
    }
    case COLOR: {
...
int MoleculeGraphics::use_material(const Material *mat) {
  ShapeClass s(MATERIAL, 9, next_id);
  float *data = s.data;
  data[0] = mat->ambient;
  data[1] = mat->specular;
  data[2] = mat->diffuse;
  ...
  data[7] = mat->transmode;
  data[8] = (float)mat->ind;
...
const char *MoleculeGraphics::info_id(int id) {
  ...
  case MATERIAL: {
    sprintf(graphics_info, "material %d", int(shape->data[8]));
    return graphics_info;
  }
  case COLOR: {
