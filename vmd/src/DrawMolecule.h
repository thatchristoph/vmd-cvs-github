 ...
 *	$Revision: 1.81 $	$Date: 2012/03/07 21:32:48 $
 *
 ***************************************************************************
 * DESCRIPTION:
 *
...
public:
  ...
  QuickSurf qsurf; ///< QuickSurf object shared by all reps in the same
                   ///< molecule.  We may have to do further refactoring
                   ///< of the code that manages the QuickSurf GPU resources
                   ///< since having multiple instances can still put 
                   ///< excessive pressure on available GPU memory.
                   ///< One solution would be to have a VMDApp method to 
                   ///< access GPU memory buffers that are only needed for
                   ///< very short-term fast-running calculations but 
                   ///< that must be completely pre-allocated to support fast
                   ///< trajectory playback, etc.
